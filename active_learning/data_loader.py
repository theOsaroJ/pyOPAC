# pyOPAC/active_learning/data_loader.py
...
class MoleculeDataset(Dataset):
    def __init__(self, descriptors: list, targets: list = None):
        ...
        if targets and len(targets) > 0:
            self.target_names = sorted(targets[0].keys())
        else:
            self.target_names = []
    ...
    def __getitem__(self, idx):
        ...
        sample = {'descriptors': torch.tensor(descriptor_values, dtype=torch.float32)}
    ...
    @property
    def output_dim(self):
        return len(self.target_names) if self.targets is not None else 0
