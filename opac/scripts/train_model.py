...
from opac.models.diffusion import train_diffusion_property_predictor, DiffusionPropertyPredictor
...
def main():
    ...
    parser.add_argument('--model-type', type=str, default='standard', choices=['standard', 'diffusion'],
                        help='Model type: "standard" (feedforward NN) or "diffusion" (probabilistic with uncertainty).')
    ...
    parser.add_argument('--num-timesteps', type=int, default=1000, help='Number of diffusion timesteps (for diffusion model).')
    ...
    if args.model_type == 'diffusion':
        logger.info("Training diffusion model for probabilistic property prediction...")
        ...
        logger.info("Training standard feedforward model...")
        model = train_model(
            dataset=train_dataset,
            input_dim=input_dim,
            output_dim=output_dim,
    ...
    if args.model_type == 'diffusion':
        model_params['num_timesteps'] = args.num_timesteps
    ...
    if args.model_type == 'diffusion':
        # For diffusion models, use uncertainty-aware evaluation
        logger.info("Evaluating diffusion model with uncertainty quantification...")
        model.eval()
        device = next(model.parameters()).device
        with torch.no_grad():
            test_descriptors = torch.stack([sample['descriptors'] for sample in test_dataset]).to(device)
            test_targets = torch.stack([sample['targets'] for sample in test_dataset]).to(device)
            
            # Get predictions with uncertainty
            mean_pred, std_pred = model.predict_with_uncertainty(test_descriptors, num_samples=10)
            
            # Compute metrics
            from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score
            mse = mean_squared_error(test_targets.cpu().numpy(), mean_pred.cpu().numpy())
            mae = mean_absolute_error(test_targets.cpu().numpy(), mean_pred.cpu().numpy())
            r2 = r2_score(test_targets.cpu().numpy(), mean_pred.cpu().numpy())
            
            logger.info(f"Test Metrics - MSE: {mse:.4f}, MAE: {mae:.4f}, R2: {r2:.4f}")
            logger.info(f"Average uncertainty (std): {std_pred.mean().item():.4f}")
    else:
        # Standard evaluation
